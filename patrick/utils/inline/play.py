...
from patrick.utils.formatters import time_to_seconds
...
def stream_markup_timer(_, videoid, chat_id, played, dur):
    ...
    patrickk = math.floor(percentage)
    if 0 < patrickk <= 10:
        bar = "✄·─·─·─·─·─·─·─·─·─"
    elif 10 < patrickk < 20:
        bar = "-ˋˏ✄·─·─·─·─·─·─·─·─"
...
def playlist_markup(_, videoid, user_id, ptype, channel, fplay):
    buttons = [
        [
            InlineKeyboardButton(
                text=_["P_B_1"],
                callback_data=f"PatrickPlaylists {videoid}|{user_id}|{ptype}|a|{channel}|{fplay}",
            ),
            InlineKeyboardButton(
                text=_["P_B_2"],
                callback_data=f"PatrickPlaylists {videoid}|{user_id}|{ptype}|v|{channel}|{fplay}",
            ),
        ],
        [
            InlineKeyboardButton(
