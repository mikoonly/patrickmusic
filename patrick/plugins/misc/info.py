...
from patrick import app as Alya
...
async def userstatus(user_id):
   try:
      user = await Alya.get_users(user_id)
      x = user.status
      if x == enums.UserStatus.RECENTLY:
        ...
        return "**Something Wrong Hapenned !**"
...
async def userinfo(_, message):
    ...
    if not message.reply_to_message and len(message.command) == 2:
        try:
            ...
            user_info = await Alya.get_chat(user_id)
            user = await Alya.get_users(user_id)
            ...
            bio = user_info.bio
            photo = await Alya.download_media(user.photo.big_file_id)
        ...
        try:
            ...
            mention = user.mention
            bio = user_info.bio
            photo = await Alya.download_media(message.reply_to_message.from_user.photo.big_file_id)
            welcome_photo = await get_userinfo_img(
                bg_path=bg_path,
                font_path=font_path,
                user_id=user_id,
